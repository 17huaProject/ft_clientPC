<template>
    <div id="orderDetail">
        ...
        <div class="jinDds">
            <!-- <div v-show="orderinfo.order_status=='UNPAID'">待支付</div> -->
            <div v-show="orderinfo.order_status=='SUCCESS'">实际付款：￥{{orderinfo.paid_amount}}</div>
            <div>订单编号：{{orderinfo.order_id}}</div>
            <div v-show="orderinfo.order_status=='SUCCESS'">支付时间：{{orderinfo.create_time}}</div>
            <div v-show="orderinfo.order_status=='UNPAID'">创建时间：{{orderinfo.create_time}}</div>
        </div>
        <div id="banTop">
            <div class="ac_name">{{orderinfo.order_name}}</div>
            <div class="ac_info">
                <div class="tips_img_onImg" v-show="orderinfo.order_status=='SUCCESS'" style="background-color:#ff7d54">可使用</div>
                <div class="tips_img_onImg" v-show="orderinfo.order_status=='UNPAID'" style="background-color:#FDA649">未支付</div>
                <div class="tips_img_onImg" v-show="orderinfo.order_status=='PENDING'" style="background-color:#C29B70">已使用</div>
                ...
                <div class="ac_info_img" :style="{background: 'url(\''+this.imgTitle+'\')'+ 'center center',backgroundSize:'auto 100%'}" ></div>
        ...
        <div id="banMiddle" v-show="orderinfo.order_status=='SUCCESS'">
            <div class="bt_text">验证码区</div>
            <div class="ticket_box" v-show="this.reunShow(0)">
                <div class="ticket_ta">
                    可使用
                    <span>（入场请出示验证码）</span>
                </div>
            ...
            <div class="ticket_box" v-show="this.reunShow(1)">
                <div class="ticket_ta_s">
                    已使用
                    <span>{{tiketTime}}</span>
                </div>
                <div class="middle_line">
                ...
                <div class="ticket_da_s">
                    {{qrCode}}
                </div>
            </div>
            <div class="ticket_box" v-show="this.reunShow(2)">
                <div class="ticket_ta_s">
                    已过期
                    <span>{{tiketTime}}</span>
                </div>
        ...
        <div id="banBottom" v-show="orderinfo.order_status=='SUCCESS'">
            <div class="jinTds">
                <div>
                    <span>订单金额：</span>
                ...
                <!--<div class="triangle-bottom_bot"></div>
                                                                    <div class="triangle-bottom"></div>-->
            </div>
    
            <!-- <div class="bt_ara"  >
                    <div class="bt_ara_div_button">继续支付</div>
                </div>
                <div class="bt_ara" >
                    <div class="bt_ara_div_finish">已关闭</div>
                </div> -->
            <!-- <div class="bt_ara" v-show="orderinfo.order_status=='SUCCESS'">
                                        <div>关闭订单</div>
                                    </div>
                                   
                                     <div class="bt_ara" v-show="orderinfo.order_status=='PENDING'" @click="rePaytheOrder()">
                                        <div>评价</div>
                                    </div> -->
        </div>
        <div class="frieWork_e" v-show="orderinfo.order_status=='UNPAID'">
            <div class="co_moution_box_div">实际付款：</div>
            <div class="co_moution_box_Money fs18">￥{{orderinfo.order_amount}}</div>
        </div>
    ...
    methods: {
        ...
        setOrderInfo(data) {
            if (data.errcode === 200) {
                ...
                this.imgTitle = this.$store.state.SyetemBaseConfig.image_prefix + data.record.order_img
