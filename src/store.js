...
const mutations = {
  ...
  saveWXconfig(state, conf) { //微信API调用配置
    ...
    var share = {
      ...
      shareimgUrl: 'https://image.17hua.me/upload/image/201709/1506503573240805.png',
    ...
    _wx.ready(function () {
      document.getElementById('lifestyle-lapse').play(); 
      _wx.getLocation({
        type: 'wgs84', // 默认为wgs84的gps坐标，如果要返回直接给openLocation用的火星坐标，可传入'gcj02'
        success: function (res) {
          state.locationIN = {
  ...
  ChangeScreamWidth(state, num) { //屏幕宽度检测loginShowStatue
    state.screamWidth = num;
  },
   ShowLoginStatue(state, statue) { //屏幕宽度检测
    state.loginShowStatue = statue;
  },
  ConectionPara(state, obj) { //请求参数补全
    let BaseArr = {
      phone: state.usrInfomation.phone || '',
      timestamp: state.serverTime.YYYYMMDDHHMMSS(),
      sign: md5(state.usrInfomation.phone || '' + state.serverTime.YYYYMMDDHHMMSS() + 'dl91XlyWBmA1GxdKYr8J0iZLbdgaCGs4'),
      pid: '200001',
      ver: '1.0',
      client: 'WEB',
  ...
  ConectionParaWithOutPhone(state, obj) { //请求参数补全
    let BaseArr = {
      timestamp: state.serverTime.YYYYMMDDHHMMSS(),
      sign: md5(state.serverTime.YYYYMMDDHHMMSS() + 'dl91XlyWBmA1GxdKYr8J0iZLbdgaCGs4'),
      pid: '200001',
      ver: '1.0',
      client: 'WEB',
      test: document.location.protocol+'//' + window.location.host
