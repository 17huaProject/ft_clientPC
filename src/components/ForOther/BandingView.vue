<template>
    <div id="manObar">
        <img id="usrImgGet" src="../../../static/obj_img/band_img/bindinglogo.png">
        <div id="usrText">与我们一起画</div>
        <div id="phoneLineSet">
...
export default {
    ...
    methods: {
        ...
        sendSMScode() {
            ...
            let getString = this.$store.state.getParamString;
            let postString = getString.replace('?', '');
            this.$http.get(this.$store.state.serverHost + '/yqhbsp/sms/sendRegister' + getString).then(m => this.$toast.bottom('短信已下发')).catch(r => console.log(r));
        },
        runGoback(data) {
            if (data.errcode == 200) {
...
#manObar {
    ...
    height: 100%;
    z-index: -1;
    background: url('../../../static/obj_img/band_img/bindingbk.png') no-repeat;
